...
export function createExecutionService(
   ...
   async function waitForConfirmation(
      ...
      for (let i = 0; i < maxAttempts; i++) {
         try {
            ...
            if (receipt.status === 'success') {
               console.log(
                  `[ExecutionService] Transaction confirmed: ${txHash}`
               );
               return true;
            } else {
               console.error(
                  `[ExecutionService] Transaction reverted: ${txHash}`
               );
               return false;
            }
         } catch {
      ...
      throw new Error(
         `Transaction not confirmed after ${maxAttempts} attempts: ${txHash}`
      );
   }
   ...
   async function executeSwapStep(
      ...
      if (!calldata || calldata === '0x') {
         console.log(
            `[ExecutionService] Building calldata for ${step.protocol}`
         );
      ...
      const tx = {
         ...
         data: calldata,
         value:
            step.fromToken === '0x0000000000000000000000000000000000000000'
   ...
   async function executeIntent(executionId: string): Promise<{
      ...
      try {
         ...
         if (!isNativeToken && swapStep.contractAddress) {
            ...
            console.log(
               `[ExecutionService] Current allowance: ${allowance}, required: ${fromAmount}`
            );

            if (allowance < fromAmount) {
               console.log(
                  `[ExecutionService] Insufficient allowance, executing approval`
               );
