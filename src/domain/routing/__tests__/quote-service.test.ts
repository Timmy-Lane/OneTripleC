...
const mockV2GetQuote = mock(() => Promise.resolve(null));
const mockV2BuildSwap = mock(() => Promise.resolve({ to: '0x' as Address, data: '0x' as Hex, value: 0n }));
const mockV3GetQuote = mock(() => Promise.resolve(null));
const mockV3BuildSwap = mock(() => Promise.resolve({ to: '0x' as Address, data: '0x' as Hex, value: 0n }));
const mockURGetQuote = mock(() => Promise.resolve(null));
const mockURBuildSwap = mock(() => Promise.resolve({ to: '0x' as Address, data: '0x' as Hex, value: 0n }));

mock.module('../../../adapters/dex/uniswap-v2-adapter.js', () => ({
   UniswapV2Adapter: class {
      getQuote = mockV2GetQuote;
      buildSwapTransaction = mockV2BuildSwap;
   },
}));

mock.module('../../../adapters/dex/uniswap-v3-adapter.js', () => ({
   UniswapV3Adapter: class {
      getQuote = mockV3GetQuote;
      buildSwapTransaction = mockV3BuildSwap;
   },
}));
...
mock.module('../../../adapters/dex/universal-router/constants.js', () => ({
   getUniversalRouterAddress: mock(() => '0x66a9893cc07d91d95644aedd05d03f95e1dba8af' as Address),
}));
...
function makeInternalQuote(overrides?: Partial<InternalSwapQuote>): InternalSwapQuote {
   return {
      fromToken: USDC,
      toToken: WETH_ADDR,
      fromAmount: 1000000n,
...
describe('QuoteService', () => {
   ...
   beforeEach(() => {
      ...
      mockV2GetQuote.mockReset();
      mockV3GetQuote.mockReset();
      mockURGetQuote.mockReset();
      mockV2GetQuote.mockResolvedValue(null);
      mockV3GetQuote.mockResolvedValue(null);
   ...
   describe('fetchQuotes (same-chain)', () => {
      ...
      test('returns quote from V3 adapter', async () => {
         const internalQuote = makeInternalQuote();
         mockV3GetQuote.mockResolvedValueOnce(internalQuote);

         const results = await service.fetchQuotes(baseRequest);

         expect(results).toHaveLength(1);
         expect(results[0].estimatedOutput).toBe(internalQuote.toAmount.toString());
         expect(results[0].route.provider).toBe('uniswap-v3');
      });

      test('returns quotes from multiple adapters', async () => {
         const v2Quote = makeInternalQuote({ protocol: 'uniswap-v2', toAmount: 490000000000000n });
         const v3Quote = makeInternalQuote({ protocol: 'uniswap-v3', toAmount: 500000000000000n });

         mockV2GetQuote.mockResolvedValueOnce(v2Quote);
         mockV3GetQuote.mockResolvedValueOnce(v3Quote);

         const results = await service.fetchQuotes(baseRequest);

         expect(results).toHaveLength(2);
         const providers = results.map(r => r.route.provider);
         expect(providers).toContain('uniswap-v2');
         expect(providers).toContain('uniswap-v3');
      });

      test('filters out failed adapters', async () => {
         const v3Quote = makeInternalQuote();
         mockV2GetQuote.mockRejectedValueOnce(new Error('RPC timeout'));
         mockV3GetQuote.mockResolvedValueOnce(v3Quote);
      ...
      test('calculates gas fee in wei', async () => {
         const internalQuote = makeInternalQuote({ estimatedGas: 200000n, fee: 3000n });
         mockV3GetQuote.mockResolvedValueOnce(internalQuote);

         const results = await service.fetchQuotes(baseRequest);

         expect(results).toHaveLength(1);
         // gasPrice = 20 gwei, gas = 200000, fee = 3000
      ...
      test('calculates USD fee using native price', async () => {
         const internalQuote = makeInternalQuote({ estimatedGas: 200000n, fee: 0n });
         mockV3GetQuote.mockResolvedValueOnce(internalQuote);
      ...
      test('includes fee breakdown in route', async () => {
         const internalQuote = makeInternalQuote({ estimatedGas: 150000n, fee: 3000n });
         mockV3GetQuote.mockResolvedValueOnce(internalQuote);
      ...
      test('falls back to 20 gwei when RPC gas price fails', async () => {
         ...
         const internalQuote = makeInternalQuote({ estimatedGas: 100000n, fee: 0n });
         mockV3GetQuote.mockResolvedValueOnce(internalQuote);
